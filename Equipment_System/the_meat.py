...
def check_out(item, date_string):
    #TODO: pyodbc.ProgrammingError: No results.  Previous SQL was not a query.
    """
    ...
    if check_in_date < (datetime.datetime.now() + datetime.timedelta(days=32)):
        ...
        query = "UPDATE Test_Equipment SET Current_Holder=?, Check_Out_Date = ?, Due_Date = ? WHERE Item_Name=?"
        result = cursor.execute(query, logged_in_user, check_out_date, formatted_check_in_date, item).fetchall()
        print(result)
        return result
